...
**Contract regression layer (V1):** the auth, rewards and playback wire shapes described below are pinned by deterministic tests in `src/services/contract/`, run against checked-in canonical fixtures derived from the backend. See `docs/v1-contract-lock.md` for what is hard-pinned, what is intentionally flexible, and how to update the fixtures when the backend changes.
...
| WhatsApp | **Implemented and tested. A PRODUCTION driver now exists** (`cloud-api`, Meta WhatsApp Cloud API) alongside the `development`/`test`-only `fake` one, so the flag is no longer confined to non-production. **No real WhatsApp message has ever been sent by this code**: the project holds no Meta credentials, so delivery itself is unproven. Enabling it needs four values from Meta's dashboards — see the backend's `docs/WHATSAPP_LOGIN_SETUP.md`. | The UI is complete and ships behind the same server-driven gate. Three server answers are handled distinctly, because the advice differs: `503 WHATSAPP_AUTH_DISABLED` (no configuration — retrying cannot help), `503 WHATSAPP_PROVIDER_UNAVAILABLE` (delivery definitively failed; **no challenge survives it**, so retry immediately and the resend button stays enabled), and `429` from either limiter (wait). **No OTP is hardcoded in any build**, and no local fake-provider capability is exposed in production UI. |
