...
- Access rule: each episode's free/premium state is read from the backend's `accessTier` field (`VideoResponseDto.accessTier`, backend commit 2f285d1) and copied onto the episode by `toEpisode` in `series-service.ts`. The client no longer derives it from the episode number - the retired `FREE_EPISODE_LIMIT`/`getEpisodeAccessType` rule is gone - so an admin override that makes an early episode premium or a late episode free is honoured, and the lock always agrees with what `GET /videos/:id/stream` will allow. **No real payment, subscription, credit balance, or purchase flow is implemented.** Selecting a premium episode shows a preview modal ("Episode ini termasuk konten premium.") with "Segera Hadir" and "Kembali ke Episode Gratis" actions only.
...
- `DramaFeedItem` has an "Episode Berikutnya" (Next Episode) control that reads the same backend-sourced access state.
